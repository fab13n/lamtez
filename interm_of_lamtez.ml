...
let rec compile_typeT ctx t =
  ...
  match t with
  | TId id -> type_error("Need to give type of "^id)
  | TLambda _ as t ->
...
let get_product_tags ctx (t:typeT) =
  ...
  let tags = match TypingContext.decl_of_name ctx name with
    | DProduct(_, _, fields) -> List.map fst fields 
    | _ -> unsound "bad product type" in
  tags

let get_sum_decl_cases ctx (t:typeT) =
  (* TODO substitute type params?! *)
  let name = match t with TApp(name, _)  -> name | _ -> unsound "bad sum type" in
  let tags = match TypingContext.decl_of_name ctx name with
    | DSum(_, _, cases) -> cases
    | _ -> unsound "bad sum type" in
  tags
...
let rec compile_exprT ctx e =
  ...
  | ETez(n)    -> IELit(sprintf "\"%d.%d\"" (n/100) (n mod 100)), it
  ...
  | ETupleGet(e, i) ->
    let n = match it with 
    | ITProduct(_, list) -> List.length (Lazy.force list) 
    | _ -> unsound "bad tuple type" in
    IEProductGet(c e, i, n), it
  ...
  | EProductGet(e, tag) ->
    ...
    IEProductGet(c e, List.assoc tag tag2num, List.length tags), it
...
let rec string_of_iType = 
  ...
  | ITProduct(_, list) -> "("^sep_list "*" t (Lazy.force list)^")"
  | ITSum(_, list) -> "("^sep_list "+" t (Lazy.force list)^")"
